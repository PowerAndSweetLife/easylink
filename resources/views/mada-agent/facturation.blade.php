...
<ul class="nav nav-underline nav-swipe mb-4">
    <li class="nav-item">
        <a @class(['nav-link', 'active' => $active === 'not-paid']) href="{{ route('mada-agent.facturation.index', ['status' => 'not-paid']) }}">{{ __("Not Paid") }}</a>
    </li>
    <li class="nav-item">
        <a @class(['nav-link', 'active' => $active === 'paid']) href="{{ route('mada-agent.facturation.index', ['status' => 'paid']) }}">{{ __("Paid") }}</a>
    </li>
</ul>
                ...
                @foreach ($lists as $item)
                    <tr>
                        ...
                        <td class="table-column-action">
                                ...
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu">
                                        @if (is_null($item->amount))
                                            <li>
                                                <a class="dropdown-item" href="{{ route('mada-agent.facturation.edit', ['id' => $item->id]) }}">
                                                    <span><i class="fa-solid fa-file-pen"></i></span>
                                                    <span class="ms-2">{{ __('Edit facture') }}</span>
                                                </a>
                                            </li>
                                        @endif
                                        <li>
                                            <a class="dropdown-item" href="{{ route('mada-agent.facturation.paiement', ['id' => $item->id]) }}">
                                                <span><i class="fa-solid fa-hand-holding-dollar"></i></span>
                                                <span class="ms-2">{{ __('Paiement') }}</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="{{ route('mada-agent.facturation.history', ['id' => $item->id]) }}">
                            ...
                            @else
                                <a class="btn btn-sm btn-outline-primary" href="{{ route('mada-agent.facturation.history', ['id' => $item->id]) }}">
                                    <span><i class="fa-solid fa-clock-rotate-left"></i></span>
                                    <span class="ms-2">{{ __('History') }}</span>
                                </a>
                            @endif
